...
class ConnectClient {
  ...
  */
  constructor(endpoint, apiKey, adapter) {
    ...
    this._tierConfigRequests = new api.TierConfigRequestService(this._adapter);
    this._actions = new api.ActionService(this._adapter);
  }
  ...
  /**
   * Returns an instance of the `ActionService` class to access
   * the product ``Action`` resource endpoint.
   *
   * @type {ActionService}
   */
  get actions() {
