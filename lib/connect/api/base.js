...
class BaseService {
  ...
  /**
   *
   * Add RQL query to querystring.
   *
   * @param {string} url The url of the API endpoint.
   * @param {Query|string} query The RQL query.
   *
   * @returns {string} The modified URL with the RQL query included in querystring.
   */

  addRQLQuery(url, query) {
    let computedUrl = url;
    if (query && (typeof query === 'string' || query instanceof Query)) {
      const separator = url.includes('?') ? '&' : '?';
      computedUrl += `${separator}${query.toString()}`;
    }
    return computedUrl;
  }
