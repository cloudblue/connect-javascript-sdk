...
class Fulfillment {
   ...
   *
   * See the :js:class:`RequestResource.list method <requests.RequestResource.list>` for
   * further information about the filters object.
   *
   * @param  {string|Query} query    A RQL query.
   ...
   * If no filter is passed, a page of Request in 'pending' status is returned.
   *
   * See the :js:class:`TierConfigRequestResource.list method <tierConfigRequests.TierConfigRequestResource.list>`
   ...
   */
  async searchTierConfigRequests(query) { /* eslint-enable max-len */
   ...
   *
   * @return  {string|null}  The Connection identifier or null.
   */
  async getConnectionIdByProductAndHub(productId, hubId) {
    ...
    const connection = _.filter(connections, (conn) => conn.hub.id === hubId);
