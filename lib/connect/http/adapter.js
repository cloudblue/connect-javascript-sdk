...
const { forEach, mergeDeepRight } = require('ramda');
...
class DefaultHttpAdapter extends AbstractHttpAdapter {
  ...
  prepareRequest(url, options) {
    if (options.body) {
      if (Array.isArray(options.body) || isObjectStrict(options.body)) {
        options.body = JSON.stringify(options.body);
        options.headers = mergeDeepRight(options.headers, { 'Content-Type': 'application/json' });
      }
    }
    return { url, options };
  }
